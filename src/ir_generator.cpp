...
void IRGenerator::generate(const ASTNode *root, std::ostream &out) {
  ...
  emit_raw("target datalayout = "
           "\"e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:"
           "64-S128\"");
  emit_raw(
      "target triple = \"x86_64-pc-linux-gnu\""); // You can adjust this triple
  emit_raw("");
...
void IRGenerator::visit(const FuncDefNode *node) {
  ...
  for (size_t i = 0; i < node->parameters.size(); ++i) {
    ...
    symbol_table_[var_name] = var_addr;
...
void IRGenerator::visit(const VarDeclNode *node) {
  ...
  // 2. Add the variable to the symbol table
  symbol_table_[node->name] = var_addr;

  // 3. If there's an initializer, compute its value and store it
  if (node->initializer) {
...
std::string IRGenerator::visit(const IdentifierNode *node) {
  // 1. Find the variable's address in the symbol table
  auto it = symbol_table_.find(node->name);
  if (it == symbol_table_.end()) {
    emit_raw("; ERROR: Undefined variable '" + node->name + "'");
    return "0"; // Should be handled by a semantic analysis pass
  }
  std::string var_addr = it->second;
...
std::string IRGenerator::visit(const AssignExprNode *node) {
  ...
  auto it = symbol_table_.find(target_node->name);
  if (it == symbol_table_.end()) {
...
std::string IRGenerator::visit(const CallExprNode *node) {
  ...
  std::string result = next_temp();
  // [TODO] We need a way to look up the function's return type.
  // For now, assuming i32 for non-void functions.
  // A proper implementation would consult a function symbol table.
